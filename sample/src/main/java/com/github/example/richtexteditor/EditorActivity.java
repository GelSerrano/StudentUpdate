...
public class EditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        storageRef = storage.getReferenceFromUrl("gs://caps-e4102.appspot.com");
    ...
    private void setUpEditor() {
        ...
        editor.setEditorListener(new EditorListener() {
            ...
            public void onUpload(Bitmap image, String uuid) {
                ...
                Log.d("uploaddata", data + " " + image.describeContents() + " uuid: " + uuid);

                imagesRef = storageRef.child("images/" + uuid);

                UploadTask uploadTask = imagesRef.putBytes(data);
                uploadTask.addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception exception) {
                        Log.d("uploadpic", ": failed" + exception);
                    ...
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        ...
                        imagesRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() { //download to firebase then pass it to editor for render
                            @Override
                            public void onSuccess(Uri downloadUrl) {
                                editor.onImageUploadComplete(String.valueOf(downloadUrl + ".jpg"), uuid);

                                Log.d("uploadserializedURI", ": " + String.valueOf(downloadUrl + ".jpg") + "");
                                // Do what you want
                            }
                        });
                ...
                /**
                 * TODO delete unused or deleted images by user on edit
                 * let the editor know the upload has completed
                 */
                //upload image to get url
                //TODO: get rendered html   editor.render("<p>Hello man, whats up!</p>");
                //        editor.render(text);

                Log.d("onUpload", image.toString() + "");
        ...
        //check if call is from New lesson or Edit lesson
        //if edit lesson, pass content here to be editted
        //on render button check it again then do conditions
        ...
        // String text = "<p>sample text</p>";


        activity_src = intent.getStringExtra("source");

        if (activity_src.equals("new_lesson")) {
            // render();
            editor.render();
            // editor.render("<p>test</p>");
            Log.d("new_lesson", "new lesson");
            findViewById(R.id.btnRender).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    String newserializedtext = editor.getContentAsSerialized();
                    //upload serialized version of text on firebase
                    //Lessons is the parent key
                    //TODO: pass lesson number here then add it to the firebase

                    String week = intent.getStringExtra("weekxtra");
                    String period = intent.getStringExtra("periodxtra");
                    String title = intent.getStringExtra("titlextra");
//                databaseReference.push().setValue(period);
                    lesson = new Lesson();
                    lesson.setSerializedtext(newserializedtext);
                    lesson.setWeek(week);
                ...
                */
                    ...
                    databaseReference.addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            databaseReference.child(period).child("week" + week).setValue(lesson);
                            Toast.makeText(EditorActivity.this, "data added", Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onCancelled(@NonNull DatabaseError error) {
                            Toast.makeText(EditorActivity.this, "Fail to add data " + error, Toast.LENGTH_SHORT).show();
                        }
                    });

                    //editor.getContentAsHTML();
//                Intent intent = new Intent(getApplicationContext(), RenderTestActivity.class);
//                intent.putExtra("content", serializedtext);
//                startActivity(intent);
                    //Todo: goto and refresh Nested lesson

                    startActivity(new Intent(EditorActivity.this, NestedLessons.class));

                }
            });

        } else if ( activity_src.equals("edit_lesson") && intent.getStringExtra("content") != null) {

             // shows editable content if edit is clicked
                String edit_content = intent.getStringExtra("content");
                editor.render(editor.getContentDeserialized(edit_content));

                String edit_week = intent.getStringExtra("edit_week"); //format is Week 10
                String wk = edit_week.toLowerCase().replaceAll("\\s", "");
                Log.d("datagetrefweek", "week is" + wk);
                findViewById(R.id.btnRender).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        //save editted to firebase
                        //search week on firebase then push update in here
                        //serializedtext
                        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                for (DataSnapshot data : snapshot.getChildren()) {
                                    ...
                                    Log.d("datagetref", "data key" + data.getKey()); //this is prelim midterm finals
                                    Log.d("datagetref", "data children" + data.getChildren());
                                    Log.d("datagetref", "snap children" + snapshot.getChildren());
                                    Log.d("datagetref", "snap key" + snapshot.getKey());

                                    for (DataSnapshot datac : data.getChildren()) {
                                        Log.d("datagetref", "week is " + wk);
                                        Log.d("datagetref", "    data key c" + datac.getKey()); //this is week10
                                        //databaseReference.child(data.getKey()).child(datac.getKey()).setValue(editserializedtext);
                                        databaseReference.child(data.getKey()).child(datac.getKey()).child("serializedtext").setValue(editor.getContentAsSerialized());
                                        Log.d("datagetref", "    data children c" + datac.getChildren());
                                        Log.d("datagetref", "    snap children data" + data.getChildren());
                                        Log.d("datagetref", "    snap key data" + data.getKey()); //this is period Prelim Midterm Finals
                                    }
